...
class ConfigurationController extends Controller
{
    ...
    public function table(string $table_name, ?string $column = null, ?string $value = null): JsonResponse
    {
        ...
        $table_name = \App\Services\InputSanitizer::sanitizeTableName($table_name);
        
        // Verificar si la tabla está en la lista de permitidas (seguridad)
        if (!\App\Services\InputSanitizer::isTableAllowed($table_name)) {
            return response()->json(['success' => false, 'message' => 'Tabla no permitida para consulta'], 403);
        }
        ...
        if ($column && $value) {
            // Sanitizar columna
            $column = \App\Services\InputSanitizer::sanitizeColumnName($column);
            
            // Verificar que la columna no sea sensible
            if (\App\Services\InputSanitizer::isSensitiveColumn($column)) {
                return response()->json(['success' => false, 'message' => 'Columna no permitida para filtro'], 403);
            }
            ...
            $value = \App\Services\InputSanitizer::sanitizeSearchValue($value);
            $query->where($column, 'LIKE', "{$value}%");
        }

        // Limitar resultados para evitar sobrecarga
        $values = $query->limit(1000)->get();
...
//                return $PrivateKey;
    ...
    public function storeCertificate(ConfigurationCertificateRequest $request)
    {
        try {
            ...
            $certificateBase64 = preg_replace('/\s+/', '', $request->certificate);
            
            if (!base64_decode($certificateBase64, true)) {
                throw new Exception('The given data was invalid.');
            }
            
            $certificateBinary = base64_decode($certificateBase64);
            
            if (!openssl_pkcs12_read($certificateBinary, $certificate, $request->password)) {
                // Recopilar errores de OpenSSL
                $errors = [];
                while ($msg = openssl_error_string()) {
                    $errors[] = $msg;
                }
                throw new Exception('The certificate could not be read. ' . implode('; ', $errors));
            }
        } catch (Exception $e) {
            $opensslErrors = [];
            while ($msg = openssl_error_string()) {
            ...
            if (!empty($opensslErrors)) {
                $errorMessage .= ' | ' . implode('; ', $opensslErrors);
            }
